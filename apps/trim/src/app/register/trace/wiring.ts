...
export const WIRING_FUNCTION: WiringFacet[] = [
  ...
  { id: "wf-entity-resolution", label: "Entity resolution", status: "deferred", flowIds: ["activate-notice"], surfaceIds: ["biz-ct-notice"] },
  {
  ...
  { id: "wf-activation", label: "Activation delivery", status: "implemented", flowIds: ["activate-notice"], surfaceIds: ["biz-ct-notice"] },
  { id: "wf-consent-auth", label: "Consent / authorization capture", status: "implemented", flowIds: ["consent-authorize"], surfaceIds: ["biz-ct-consent", "biz-ct-authorize"] },
  { id: "wf-docs", label: "Doc intake", status: "partial", flowIds: ["consent-authorize"], surfaceIds: ["biz-ct-upload"], notes: "Fixed checklist, not yet adaptive to entity history." },
  { id: "wf-claim-sm", label: "Claim state machine", status: "partial", flowIds: ["file-claim-packet", "recover-kickback"], surfaceIds: ["biz-ct-status", "agency-ct-exceptions"], notes: "Missing needs_reinstatement / reconciling states." },
  { id: "wf-filing", label: "Filing", status: "implemented", flowIds: ["file-claim-packet"], surfaceIds: ["biz-ct-status"] },
  { id: "wf-followup", label: "Follow-up / resubmission", status: "implemented", flowIds: ["recover-kickback"], surfaceIds: ["agency-ct-exceptions"] },
  { id: "wf-remittance-reconcile", label: "Remittance reconcile", status: "deferred", flowIds: ["close-paid-invoice"], surfaceIds: ["biz-ct-paid"] },
  { id: "wf-invoice", label: "Invoice", status: "implemented", flowIds: ["close-paid-invoice"], surfaceIds: ["biz-ct-invoice"] },
  { id: "wf-audit", label: "Audit", status: "implemented", flowIds: [], surfaceIds: ["agency-ct-audit"] },
...
export const WIRING_CANTS: WiringFacet[] = [
  ...
  { id: "wc-notary", label: "Notarized/mail-original filing branch", status: "deferred", flowIds: ["file-claim-packet"], surfaceIds: ["agency-ct-jurisdiction"] },
  { id: "wc-fee-caps", label: "Per-state (and time-in-custody-dependent) fee-cap table", status: "deferred", flowIds: ["consent-authorize"], surfaceIds: ["agency-ct-jurisdiction", "biz-ct-authorize"] },
  { id: "wc-1099", label: "Trove's own W-9/1099 vendor-side process", status: "deferred", flowIds: [], surfaceIds: [] },
  { id: "wc-bank-confirm", label: "Bank/payout confirmation mechanism without touching funds", status: "deferred", flowIds: ["close-paid-invoice"], surfaceIds: ["biz-ct-paid"] },
  { id: "wc-dlq", label: "Dead-letter-queue replay for stuck filing/follow-up jobs", status: "deferred", flowIds: ["recover-kickback"], surfaceIds: ["agency-ct-exceptions"] },
...
export const WIRING_FURNISH: WiringFacet[] = [
  { id: "wfu-job-health", label: "Job health", status: "deferred", flowIds: [], surfaceIds: ["agency-ct-audit"] },
  { id: "wfu-cost-glance", label: "Cost glance", status: "deferred", flowIds: [], surfaceIds: ["agency-ct-portfolio"] },
  { id: "wfu-replay", label: "Replay control", status: "deferred", flowIds: ["recover-kickback"], surfaceIds: ["agency-ct-exceptions"] },
  { id: "wfu-audit-export", label: "Audit export", status: "deferred", flowIds: [], surfaceIds: ["agency-ct-audit"] },
];
...
export const CROSS_CUTTING_FINDINGS: CrossCuttingFinding[] = [
  {
    ...
    productSide: "biz-authorize currently implies a flat, known percentage the officer signs off on; the real range is 5%–30% depending on state and time-in-custody.",
    ...
    reconciliationNeeded: "PM's \"officer sees one number, signs once\" UX and CTO's \"fee is computed, not configured\" system requirement must be designed together.",
    surfaceIds: ["biz-ct-authorize", "agency-ct-jurisdiction"],
  ...
  },
  {
    ...
    surfaceIds: ["agency-ct-jurisdiction", "biz-ct-notice"],
    flowIds: ["activate-notice"],
  },
  {
    id: "cc-3",
    rank: 3,
    title: "Money never touches Trove — resolves transmitter risk, reopens collection risk",
    productSide: "Validates the no-escrow design and paid_claimant → invoiced sequence, but the officer may feel friction at the invoice step after already being paid in full.",
  ...
  },
  {
    ...
    surfaceIds: ["biz-ct-status", "biz-ct-notice"],
    flowIds: ["activate-notice"],
  },
  {
    id: "cc-6",
    ...
    reconciliationNeeded: "PM's claim-state vocabulary needs a state the Agency Owner can use internally without ever surfacing denial-shaped language for this situation.",
    surfaceIds: ["biz-ct-status", "agency-ct-exceptions"],
  ...
  },
  {
    ...
    productSide: "The full recovered amount is likely taxable income in the period received; Trove's invoice is a separate, possibly later-year deductible expense.",
    systemSide: "paid_claimant and invoiced need distinct, accurately-dated facts precisely because the two dates may fall in different tax periods.",
    ...
    surfaceIds: ["biz-ct-consent", "biz-ct-paid", "biz-ct-invoice"],
    flowIds: ["close-paid-invoice"],
  },
  {
    id: "cc-8",
  ...
  },
  {
    ...
    productSide: "The actual reason codes a Business Officer or Agency Owner sees aren't sourced from any single authoritative cross-state list — Trove has to define its own.",
